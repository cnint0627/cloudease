...
Page({
  data:{
    ...
    dataMessages:[
      {"role":"user","content":"请扮演一名心理咨询师，一步一步提问引导我，一次只提一个问题"},
  ...
  formSubmit(e){
    ...
    var dataMessages=this.data.dataMessages
    if(userMsg==''){
      wx.showToast({
        icon:"error",
        title: '不能发送空内容',
      ...
      success:res=>{
        ...
        wx.cloud.callFunction({
          ...
          complete:res=>{
            ...
            dataMessages.push({"role":"assistant","content":res.result.AIMsg})
            this.setData({
              dataMessages:dataMessages,
              messages:messages,
              isSend:false
  ...
  onUnload(){
    if(this.data.dataMessages.length>2){
    console.log("上传聊天记录")
    wx.cloud.callFunction({
      name:"uploadAIChatRecord",
      data:{
        dataMessages:this.data.dataMessages
      },
      complete:res=>{
        console.log(this.data.dataMessages)
        console.log("上传成功！")
